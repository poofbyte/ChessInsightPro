      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Free play against yourself or explore training scenarios.</p>
          </div>
        </div>
          ...
          <div className="max-w-xl mx-auto space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setUseScenario(false)} className={`p-4 rounded-2xl border text-left transition ${!useScenario ? "border-indigo-500 bg-indigo-500/10" : "border-slate-800 bg-[#0d1326] hover:border-slate-600"}`}>
                  <span className="font-bold text-white">Standard Start</span>
                  <p className="text-xs text-slate-400 mt-1">Begin from the opening position.</p>
                </button>
                <button onClick={() => setUseScenario(true)} className={`p-4 rounded-2xl border text-left transition ${useScenario ? "border-indigo-500 bg-indigo-500/10" : "border-slate-800 bg-[#0d1326] hover:border-slate-600"}`}>
                  <span className="font-bold text-white">Training Scenario</span>
                  <p className="text-xs text-slate-400 mt-1">Start from a curated position.</p>
                </button>
              </div>
            </div>
            ...
            {useScenario && (
              <div className="space-y-2">
                {SCENARIOS.map((s) => (
                  <button key={s.id} onClick={() => setSelectedScenario(s)} className={`w-full p-4 rounded-2xl border text-left transition ${selectedScenario.id === s.id ? "border-indigo-500 bg-indigo-500/10" : "border-slate-800 bg-[#0d1326] hover:border-slate-600"}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(["white", "black"] as const).map((c) => (
                  <button key={c} onClick={() => setOrientation(c)} className={`p-3 rounded-xl border flex items-center gap-2 transition ${orientation === c ? "border-indigo-500 bg-indigo-500/10" : "border-slate-800 bg-[#0d1326] hover:border-slate-600"}`}>
                    <div className={`w-6 h-6 rounded-full border-2 ${c === "white" ? "bg-white border-slate-300" : "bg-slate-900 border-slate-600"}`} />
                    <span className="font-bold text-white capitalize text-sm">{c}</span>
                  </button>
                ))}
              </div>
            </div>

            <button onClick={start} className="w-full py-4 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-black rounded-2xl hover:from-indigo-400 transition shadow-lg shadow-indigo-500/20">
              Start Practice
            </button>
          </div>
        )}
          ...
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-5 flex flex-col gap-4">
              ...
              <div className="p-5 bg-[#0d1326] border border-slate-800 rounded-2xl">
                ...
                <div className="p-3 bg-slate-900/60 rounded-xl text-sm text-slate-300 leading-relaxed flex gap-2">
                  <MessageSquare className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
                  {tip}
                </div>
              </div>
              {/* Game info */}
              <div className="p-4 bg-[#0d1326] border border-slate-800 rounded-xl space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-400">Turn</span>
                  <span className="font-bold text-white">{chess.turn() === "w" ? "White" : "Black"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Move</span>
                  <span className="font-bold text-white">{chess.history().length}</span>
                </div>
                {chess.isCheck() && (
