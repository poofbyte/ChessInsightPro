      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Everything you need to know to play — from movement to special rules.</p>
          </div>
        </div>

        {/* Chapter tabs */}
        <div className="flex gap-1 mb-8 bg-[#0d1326] border border-slate-800 p-1 rounded-2xl overflow-x-auto">
          {RULES.map((r, i) => (
            <button
              key={r.id}
              onClick={() => setIdx(i)}
              className={`px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition ${i === idx ? "bg-amber-500 text-black" : "text-slate-400 hover:text-white"}`}
            >
              {r.icon} {r.title}
            </button>
          ))}
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-6 flex flex-col gap-5 justify-center">
            <div>
              ...
              <div className="prose prose-invert max-w-none">
                {rule.content.split("\n").map((line, i) => {
                  ...
                  return <p key={i} className={`text-sm leading-relaxed mb-2 ${isBold ? "font-black text-amber-400" : "text-slate-300"}`}>{line}</p>;
                })}
              </div>
            </div>
            {/* Tip */}
            <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl">
              ...
              <p className="text-sm text-slate-300 mt-2 leading-relaxed">{rule.tip}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-8">
          <button disabled={idx === 0} onClick={() => setIdx(i => i - 1)} className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 rounded-xl text-sm font-bold transition">
            <ChevronLeft className="w-4 h-4" /> Previous
          </button>
          <span className="text-xs text-slate-500">{idx + 1} / {RULES.length}</span>
          <button disabled={idx === RULES.length - 1} onClick={() => setIdx(i => i + 1)} className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 rounded-xl text-sm font-bold transition">
